...
const RentalDetails = () => {
  ...
  useEffect(() => {
    const resetQuantity = async () => {
      ...
      if (rental && rental._id) {
        try {
          await fetch(`http://localhost:6001/rental/reset-quantity/${rental._id}`, {
            method: 'PATCH',
          });
        } catch (error) {
  ...
  const handleIncrease = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:6001/rental/${rental._id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleAddToCart = () => {
    if (user && user.email) {
        ...
        if (selectedDays > 1) {  // Only include days if more than 1 day is selected
          ...
          <div className="bg-gradient-to-r from-gray-600 via-gray-900 to-gray-900 md:min-h-[600px] grid items-start grid-cols-1 lg:grid-cols-5 md:grid-cols-2">
            ...
            <div className="lg:col-span-3 h-full p-8">
              <div className="relative h-full flex items-center justify-center lg:min-h-[580px]">
                <img src={rental.image} alt="Product" className="lg:w-3/5 w-3/4 h-full object-contain max-lg:p-8" />
                ...
                <div className="flex space-x-4 items-end absolute right-0 max-md:right-4 md:bottom-4 bottom-0">
                  <div className="bg-white w-10 h-10 grid items-center justify-center rounded-full rotate-90 shrink-0 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 fill-[#333] inline" viewBox="0 0 24 24">
                      <path d="M11.99997 18.1669a2.38 2.38 0 0 1-1.68266-.69733l-9.52-9.52a2.38 2.38 0 1 1 3.36532-3.36532l7.83734 7.83734 7.83734-7.83734a2.38 2.38 0 1 1 3.36532 3.36532l-9.52 9.52a2.38 2.38 0 0 1-1.68266.69734z" data-original="#000000"></path>
                    </svg>
                  </div>
            ...
            <div className="lg:col-span-2 bg-gray-100 py-6 px-8 h-full">
              <div>
                <h2 className="text-2xl font-bold text-gray-800">{rental.name}</h2>

                <div className="flex space-x-1 mt-2">
                  <svg className="w-4 fill-orange-500" viewBox="0 0 14 13" fill="none"
              ...
              <div className="mt-8">
                <h3 className="text-lg font-bold text-gray-800">Rental Price</h3>
                <p className="text-gray-800 text-3xl font-bold my-4">
                  ₱{rentalPrice} <span className="text-lg text-gray-600">for {selectedDays} day(s)</span>
                </p>
                {/* Open the modal using document.getElementById('ID').showModal() method */}
                <button className="link link-primary" onClick={() => document.getElementById('modifydate').showModal()}>How many days would you rent this item?</button>
                <dialog id="modifydate" className="modal modal-bottom sm:modal-middle">
                  <div className="modal-box">
                    <h3 className="font-bold text-2xl">Select How many days would you rent the item</h3>
                    <select
              ...
              <div className="mt-8">
                ...
                <div className="flex divide-x border w-max mt-4 rounded overflow-hidden">
                  <button
                    onClick={() => handleDecrease()} disabled={isLoading || rental.quantity <= 1}
                    type="button"
                  ...
                  <button
                    onClick={() => handleIncrease()} disabled={isLoading}
                    type="button"
