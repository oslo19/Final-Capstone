...
const CompletedOrders = () => {
  ...
  const { data: completedOrders = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${BASE_URL}/orders?source=cart&status=completed`, {
        headers: {
  ...
  if (isLoading) return <p className="text-center mt-10">Loading completed orders..</p>;
  if (isError)
    return <p className="text-center text-red-500 mt-10">Error fetching completed orders. Please try again later.</p>;
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="w-full max-w-6xl bg-white shadow-md border border-gray-300 rounded-md">
        ...
        <div className="p-4 bg-gray-100 border-b border-gray-300">
          <h2 className="text-lg font-semibold text-gray-700 text-center">Completed Orders</h2>
        </div>
