...
const ConfirmOrders = () => {
  ...
  const { data: confirmedOrders = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${BASE_URL}/orders?source=cart&status=confirmed`, {
        headers: {
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="w-full max-w-6xl bg-white shadow-md border border-gray-300 rounded-md">
        ...
        <div className="p-4 bg-gray-100 border-b border-gray-300">
          <h2 className="text-lg font-semibold text-gray-700 text-center">Confirmed Orders</h2>
        </div>

        {/* Table */}
        <div className="overflow-x-auto">
          {confirmedOrders.length === 0 ? (
            <p className="text-center py-10 text-gray-600">No confirmed orders.</p>
          ) : (
