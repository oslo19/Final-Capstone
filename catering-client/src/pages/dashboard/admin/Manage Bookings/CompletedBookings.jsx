...
const CompletedBookings = () => {
  ...
  const { data: completedOrders = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${BASE_URL}/orders?source=booking&status=completed`, {
        headers: {
  ...
  if (isLoading) return <p className="text-center mt-10">Loading completed bookings...</p>;
  if (isError)
    return <p className="text-center text-red-500 mt-10">Error fetching completed bookings. Please try again later.</p>;
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="w-full max-w-6xl bg-white shadow-md border border-gray-300 rounded-md">
        ...
        <div className="p-4 bg-gray-100 border-b border-gray-300">
          <h2 className="text-lg font-semibold text-gray-700 text-center">Completed Bookings</h2>
        </div>

        {/* Table */}
        <div className="overflow-x-auto">
          {completedOrders.length === 0 ? (
            <p className="text-center py-10 text-gray-600">No completed bookings.</p>
          ) : (
        ...
        <div className="p-4 bg-gray-100 border-t border-gray-300 text-sm text-gray-500 text-center">
          Total Completed Bookings: {completedOrders.length}
        </div>
      </div>
    </div>
  );
