...
const ConfirmBookings = () => {
  ...
  const handleEditOrder = (order) => {
    setSelectedOrder(order); // Set the selected order data for editing
    setIsModalOpen(true); // Open the modal

    // Add rental items to the selectedOrder object before opening the modal
    setSelectedOrder({
      ...order,
      rentalItems: order.items.venueItems || [], // Assuming venueItems represent rental items
    });
  };
  ...
  const filteredOrders = useMemo(() => {
    const filteredByStatusAndSource = allOrders.filter(
      (order) => order.source === "booking" && order.status === "confirmed"
    );
  ...
  const handleUpdateOrder = async (updatedOrder) => {
    try {
      const res = await fetch(`${BASE_URL}/orders/${updatedOrder._id}`, {
        method: "PUT", // Update order status and payment
        headers: {
          authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
        ...
        <div className="overflow-x-auto">
          {filteredOrders.length === 0 ? (
            <p className="text-center py-10 text-gray-600">
              No confirm orders from cart available.
            </p>
          ) : (
